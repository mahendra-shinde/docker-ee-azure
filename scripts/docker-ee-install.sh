...
wget  -qO - https://repos.mirantis.com/ubuntu/gpg | apt-key add -  
...
echo "docker container run --rm -it --name ucp  -v /var/run/docker.sock:/var/run/docker.sock mirantis/ucp:3.3.2 install --host-address 10.0.0.4 --san=$FQDN --admin-password=$ADMINPASS --admin-username=$USERNAME" >> /home/$USERNAME/.bashrc
